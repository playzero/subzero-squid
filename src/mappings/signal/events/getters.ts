...
export function getProposalCreatedData(ctx: Context, ev: Event): ProposalCreatedData {
    ...
    if (event.isV74) {
        const { account, proposalId, orgId, campaignId, amount, start, expiry } = event.asV74
...
export function getProposalVotedData(ctx: Context, ev: Event): ProposalVotedData {
    ...
    if (event.isV74) {
        const { account, proposalId, voted, yes, no, votePower } = event.asV74
