...
import { getProposalAcceptedData } from './getters'
...
async function handleProposalAcceptedEvent(ctx: EventHandlerContext, block: SubstrateBlock, event: Event) {
	let proposalIdArray = getProposalAcceptedData(ctx, event)
