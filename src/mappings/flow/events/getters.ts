...
export function getCampaignCreatedData(ctx: Context, ev: Event): CampaignCreatedData {
    ...
    if (event.isV74) {
        const { campaignId, creator, admin, target, deposit, expiry, name } = event.asV74
...
export function getCampaignContributedData(ctx: Context, ev: Event): CampaignContributedData {
    ...
    if (event.isV74) {
        const { campaignId, sender, contribution, blockNumber } = event.asV74
...
export function getCampaignFailedData(ctx: Context, ev: Event): Uint8Array {
    ...
    if (event.isV74) {
        const { campaignId, campaignBalance, blockNumber } = event.asV74
        return campaignId
    } else {
        throw new UnknownVersionError(event.constructor.name)
    }
